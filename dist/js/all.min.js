    ...
    initializeContentFeatures(appName) {
        switch (appName) {
            ...
            case 'about':
                ...
                break;
            case 'calculator':
                this.initializeCalculatorFeatures();
                break;
            case 'settings':
        ...
        // Emit contentLoaded event for any additional initialization
        document.dispatchEvent(new CustomEvent('contentLoaded', {
            detail: { appName: appName }
        }));
    },

    // Initialize calculator-specific features
    initializeCalculatorFeatures() {
        // Let the HealthcareMarketingCalculator handle initialization
        if (window.healthcareCalculator) {
            setTimeout(() => {
                if (!window.healthcareCalculator.isInitialized) {
                    window.healthcareCalculator.initializeCalculator();
                }
            }, 100);
        }
    },
...
class SettingsManager {
    ...
    loadWallpapers() {
        ...
        this.wallpapers.forEach(wallpaper => {
            ...
            // Create image element directly - no lazy loading complications
            wallpaperOption.innerHTML = `
                <img src="images/wallpapers/${wallpaper.file}" 
                     alt="${wallpaper.name}" 
                     class="wallpaper-preview"
                     width="120" 
                     height="80"
                     style="width: 120px !important; height: 80px !important; flex-shrink: 0; display: block; object-fit: cover;">
        ...
        console.log('Wallpapers loaded:', this.wallpapers.length);
    }

    // Simplified initialization - no complex lazy loading
    initProgressiveLazyLoading() {
        // This method is now simplified and just logs for debugging
        console.log('Wallpaper thumbnails should now be loading directly');
    }
...
// Healthcare Marketing Calculator Implementation
class HealthcareMarketingCalculator {
    constructor() {
        this.currentCalculator = 'dashboard';
    ...
    setupEventListeners() {
        ...
        document.addEventListener('contentLoaded', (e) => {
            if (e.detail && e.detail.appName === 'calculator') {
                setTimeout(() => this.initializeCalculator(), 100);
            }
        });

        // Also try immediate initialization if calculator content already exists
    ...
    initializeCalculator() {
        ...
        this.showCalculator('dashboard');
        
        // Load saved history
        this.loadCalculationHistory();
        
        this.isInitialized = true;
        console.log('Healthcare Marketing Calculator initialized successfully');
    }
    ...
    setupFormHandlers() {
        ...
        if (pacForm) {
            pacForm.addEventListener('submit', (e) => {
                e.preventDefault();
                this.calculatePAC();
            });
        }
        ...
        if (ltvForm) {
            ltvForm.addEventListener('submit', (e) => {
                e.preventDefault();
                this.calculateLTV();
            });
        }
    ...
    setupRealTimeUpdates() {
        const inputs = document.querySelectorAll('.calculator-section input[type="number"]');
        inputs.forEach(input => {
    ...
    showCalculator(calculatorId) {
        ...
        const selectedSection = document.getElementById(`${calculatorId}-calculator`);
        if (selectedSection) {
    ...
    calculateROI() {
        ...
        const investment = parseFloat(form.querySelector('input[name="investment"]')?.value) || 0;
        const revenue = parseFloat(form.querySelector('input[name="revenue"]')?.value) || 0;
        const timeframe = parseInt(form.querySelector('select[name="timeframe"]')?.value) || 12;

        if (investment === 0) return;

        const roi = ((revenue - investment) / investment) * 100;
        const monthlyROI = roi / timeframe;
        const profitMargin = ((revenue - investment) / revenue) * 100;

        // Update results
        this.updateROIResults({
            roi: roi,
        ...
        // Add to history
        this.addToHistory('ROI', {
            type: 'ROI Analysis',
            investment: this.formatCurrency(investment),
            revenue: this.formatCurrency(revenue),
            roi: roi.toFixed(2) + '%',
            netProfit: this.formatCurrency(revenue - investment)
        });
    }

    updateROIResults(results) {
        const container = document.getElementById('roi-results');
        if (!container) return;
            ...
            <div class="result-card ${roiClass}">
                ...
                <div class="metric-grid">
                    <div class="metric">
                        ...
                        <span class="metric-value">${results.roi.toFixed(2)}%</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">Monthly ROI</span>
                        <span class="metric-value">${results.monthlyROI.toFixed(2)}%</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">Net Profit</span>
                        <span class="metric-value">${this.formatCurrency(results.netProfit)}</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">Profit Margin</span>
                        <span class="metric-value">${results.profitMargin.toFixed(2)}%</span>
                    </div>
                </div>
                ${this.generateROIRecommendations(results)}
            </div>
        `;
    }
    ...
    calculatePAC() {
        ...
        const marketingSpend = parseFloat(form.querySelector('input[name="marketing-spend"]')?.value) || 0;
        const newPatients = parseInt(form.querySelector('input[name="new-patients"]')?.value) || 0;
        const specialty = form.querySelector('select[name="specialty"]')?.value || 'general';

        if (newPatients === 0) return;

        const pac = marketingSpend / newPatients;
        const industryBenchmark = this.getIndustryBenchmark(specialty);
        const performance = pac <= industryBenchmark ? 'excellent' : pac <= industryBenchmark * 1.5 ? 'good' : 'needs-improvement';

        this.updatePACResults({
            pac: pac,
        ...
        this.addToHistory('PAC', {
            type: 'Patient Acquisition Cost',
            specialty: specialty,
            pac: this.formatCurrency(pac),
            benchmark: this.formatCurrency(industryBenchmark),
            performance: performance
        });
    }

    updatePACResults(results) {
        const container = document.getElementById('pac-results');
        if (!container) return;

        container.innerHTML = `
            <div class="result-card ${results.performance}">
                <h3>Patient Acquisition Cost Analysis</h3>
                <div class="metric-grid">
                    <div class="metric">
                        <span class="metric-label">Your PAC</span>
                        <span class="metric-value">${this.formatCurrency(results.pac)}</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">Industry Benchmark</span>
                        <span class="metric-value">${this.formatCurrency(results.benchmark)}</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">Performance</span>
                        <span class="metric-value performance-${results.performance}">${results.performance.replace('-', ' ')}</span>
                    </div>
                </div>
                ${this.generatePACRecommendations(results)}
            </div>
        `;
    }
    ...
    calculateLTV() {
        ...
        const avgTransactionValue = parseFloat(form.querySelector('input[name="avg-transaction"]')?.value) || 0;
        const visitsPerYear = parseInt(form.querySelector('input[name="visits-per-year"]')?.value) || 0;
        const retentionYears = parseFloat(form.querySelector('input[name="retention-years"]')?.value) || 0;
        const referralRate = parseFloat(form.querySelector('input[name="referral-rate"]')?.value) || 0;

        const baseLTV = avgTransactionValue * visitsPerYear * retentionYears;
        const referralValue = baseLTV * (referralRate / 100);
        const totalLTV = baseLTV + referralValue;

        this.updateLTVResults({
            baseLTV: baseLTV,
        ...
        this.addToHistory('LTV', {
            type: 'Lifetime Value',
            baseLTV: this.formatCurrency(baseLTV),
            totalLTV: this.formatCurrency(totalLTV),
            referralValue: this.formatCurrency(referralValue)
        });
    }

    updateLTVResults(results) {
        const container = document.getElementById('ltv-results');
        if (!container) return;

        container.innerHTML = `
            <div class="result-card">
                <h3>Patient Lifetime Value Analysis</h3>
                <div class="metric-grid">
                    <div class="metric">
                        <span class="metric-label">Base LTV</span>
                        <span class="metric-value">${this.formatCurrency(results.baseLTV)}</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">Referral Value</span>
                        <span class="metric-value">${this.formatCurrency(results.referralValue)}</span>
                    </div>
                    <div class="metric highlight">
                        <span class="metric-label">Total LTV</span>
                        <span class="metric-value">${this.formatCurrency(results.totalLTV)}</span>
                    </div>
                </div>
                ${this.generateLTVRecommendations(results)}
            </div>
        `;
    }
    ...
    calculateConversion() {
        ...
        const visitors = parseInt(form.querySelector('input[name="visitors"]')?.value) || 0;
        const leads = parseInt(form.querySelector('input[name="leads"]')?.value) || 0;
        const appointments = parseInt(form.querySelector('input[name="appointments"]')?.value) || 0;
        const patients = parseInt(form.querySelector('input[name="patients"]')?.value) || 0;

        if (visitors === 0) return;

        const visitorToLead = (leads / visitors) * 100;
        const leadToAppointment = leads > 0 ? (appointments / leads) * 100 : 0;
        const appointmentToPatient = appointments > 0 ? (patients / appointments) * 100 : 0;
        const overallConversion = (patients / visitors) * 100;

        this.updateConversionResults({
            visitorToLead: visitorToLead,
            leadToAppointment: leadToAppointment,
            appointmentToPatient: appointmentToPatient,
        ...
        this.addToHistory('Conversion', {
            ...
            visitorToLead: visitorToLead.toFixed(2) + '%',
            leadToAppointment: leadToAppointment.toFixed(2) + '%',
            appointmentToPatient: appointmentToPatient.toFixed(2) + '%'
        });
    }

    updateConversionResults(results) {
            ...
            <div class="result-card">
                ...
                <div class="funnel-visualization">
                    <div class="funnel-stage">
                        <div class="stage-label">Visitors</div>
                        <div class="stage-value">${results.visitors.toLocaleString()}</div>
                        <div class="stage-bar" style="width: 100%"></div>
                    </div>
                    <div class="funnel-stage">
                        <div class="stage-label">Leads</div>
                        <div class="stage-value">${results.leads.toLocaleString()} (${results.visitorToLead.toFixed(1)}%)</div>
                        <div class="stage-bar" style="width: ${Math.max(results.visitorToLead * 2, 10)}%"></div>
                    </div>
                    <div class="funnel-stage">
                        <div class="stage-label">Appointments</div>
                        <div class="stage-value">${results.appointments.toLocaleString()} (${results.leadToAppointment.toFixed(1)}%)</div>
                        <div class="stage-bar" style="width: ${Math.max(results.leadToAppointment * 2, 10)}%"></div>
                    </div>
                    <div class="funnel-stage">
                        <div class="stage-label">Patients</div>
                        <div class="stage-value">${results.patients.toLocaleString()} (${results.appointmentToPatient.toFixed(1)}%)</div>
                        <div class="stage-bar" style="width: ${Math.max(results.appointmentToPatient * 2, 10)}%"></div>
                    </div>
                </div>
                <div class="overall-conversion">
                    <span class="metric-label">Overall Conversion Rate</span>
                    <span class="metric-value highlight">${results.overallConversion.toFixed(2)}%</span>
                </div>
                ${this.generateConversionRecommendations(results)}
            </div>
        `;
    }
    ...
    calculateBudget() {
        ...
        const totalBudget = parseFloat(form.querySelector('input[name="total-budget"]')?.value) || 0;
        const practiceType = form.querySelector('select[name="practice-type"]')?.value || 'general';
        const timeline = parseInt(form.querySelector('select[name="timeline"]')?.value) || 12;

        if (totalBudget === 0) return;

        const allocation = this.calculateBudgetAllocation(totalBudget, practiceType);
        const monthlyBudget = totalBudget / timeline;

        this.updateBudgetResults({
        ...
        this.addToHistory('Budget', {
            type: 'Budget Planning',
            totalBudget: this.formatCurrency(totalBudget),
            monthlyBudget: this.formatCurrency(monthlyBudget),
            practiceType: practiceType,
            ...
            <div class="result-card">
                ...
                <div class="budget-overview">
                    <div class="metric">
                        ...
                        <span class="metric-value">${this.formatCurrency(results.totalBudget)}</span>
                    </div>
                    <div class="metric">
                        <span class="metric-label">Monthly Budget</span>
                        <span class="metric-value">${this.formatCurrency(results.monthlyBudget)}</span>
                    </div>
                </div>
                <div class="allocation-breakdown">
                    ${Object.entries(results.allocation).map(([category, amount]) => `
                        <div class="allocation-item">
                            <span class="allocation-category">${category}</span>
                            <span class="allocation-amount">${this.formatCurrency(amount)}</span>
                            <span class="allocation-percentage">${((amount / results.totalBudget) * 100).toFixed(1)}%</span>
                        </div>
                    `).join('')}
                </div>
                ${this.generateBudgetRecommendations(results)}
            </div>
        `;
    }
    ...
    getIndustryBenchmark(specialty) {
        const benchmarks = {
            'general': 150,
            'dental': 200,
            'cardiology': 300,
        ...
        return benchmarks[specialty] || 150;
    }

    calculateBudgetAllocation(totalBudget, practiceType) {
        const allocations = {
            'general': {
                'SEO & Content Marketing': 0.35,
                'Paid Advertising': 0.30,
                'Social Media Marketing': 0.15,
                'Website Development': 0.10,
            ...
            'dental': {
                ...
                'Google Ads': 0.25,
                'Social Media': 0.20,
                'Website Optimization': 0.10,
                'Review Management': 0.05
            },
            'specialist': {
                'Paid Search': 0.35,
                'SEO': 0.30,
        ...
        const allocation = allocations[practiceType] || allocations['general'];
        const result = {};

        Object.entries(allocation).forEach(([category, percentage]) => {
            result[category] = totalBudget * percentage;
        });

        return result;
    }

    generateROIRecommendations(results) {
        const recommendations = [];
        
        if (results.roi < 0) {
            recommendations.push("🔴 Negative ROI: Review and optimize your marketing strategy immediately");
            recommendations.push("💡 Focus on high-converting channels and reduce underperforming spend");
        } else if (results.roi < 100) {
            recommendations.push("🟡 ROI below 100%: Consider optimization opportunities");
            recommendations.push("📊 Analyze conversion funnel for improvement areas");
        } else {
            recommendations.push("🟢 Excellent ROI performance!");
            recommendations.push("🚀 Consider scaling successful campaigns");
        }

        if (results.profitMargin < 20) {
            recommendations.push("⚠️ Low profit margin: Focus on increasing average transaction value");
        }
    ...
    generatePACRecommendations(results) {
        const recommendations = [];
        
        if (results.performance === 'excellent') {
            recommendations.push("🟢 Your PAC is below industry benchmark - excellent performance!");
            recommendations.push("🚀 Consider increasing budget to scale acquisition");
        } else if (results.performance === 'good') {
            recommendations.push("🟡 PAC is reasonable but has room for improvement");
            recommendations.push("🎯 Focus on targeting optimization and ad quality");
        } else {
            recommendations.push("🔴 PAC is above industry benchmark - optimization needed");
            recommendations.push("📊 Review targeting, messaging, and conversion optimization");
            recommendations.push("🔍 Analyze your highest-performing channels and focus budget there");
        }
    ...
    generateLTVRecommendations(results) {
        const recommendations = [];
        
        if (results.totalLTV > 2000) {
            recommendations.push("🟢 Strong LTV indicates healthy patient relationships");
        } else {
            recommendations.push("📈 Focus on improving patient retention and value");
        }

        if (results.referralRate < 20) {
            recommendations.push("🤝 Implement referral programs to increase patient advocacy");
        }

        recommendations.push("💡 Consider membership plans or preventive care packages");
        recommendations.push("📧 Develop patient education content to improve retention");
    ...
    generateConversionRecommendations(results) {
        const recommendations = [];
        
        if (results.visitorToLead < 2) {
            recommendations.push("🎯 Improve landing page optimization and CTAs");
            recommendations.push("📱 Ensure mobile-friendly user experience");
        }

        if (results.leadToAppointment < 30) {
            recommendations.push("📞 Optimize appointment booking process");
            recommendations.push("⏰ Implement immediate callback systems");
        }

        if (results.appointmentToPatient < 80) {
            recommendations.push("🏥 Review consultation process and patient experience");
            recommendations.push("💬 Improve pre-appointment communication");
        }
    ...
    generateBudgetRecommendations(results) {
        const recommendations = [];
        
        recommendations.push("📊 Monitor performance of each channel monthly");
        recommendations.push("🔄 Reallocate budget based on ROI performance");
        recommendations.push("📈 Track leading indicators like traffic and lead quality");
        
        if (results.totalBudget < 5000) {
            recommendations.push("💡 Focus on high-impact, low-cost strategies first");
            recommendations.push("🎯 Prioritize local SEO and organic social media");
        } else {
            recommendations.push("🚀 Consider advanced strategies like remarketing and automation");
        }
    ...
    updateHistoryDisplay() {
        ...
        if (this.calculationHistory.length === 0) {
            container.innerHTML = '<p>Start using the calculators to see your history here</p>';
            return;
        }
        ...
        container.innerHTML = `
            <div class="history-header">
                ...
                <button onclick="window.healthcareCalculator.clearHistory()" class="clear-history-btn">Clear History</button>
            </div>
            <div class="history-list">
                ${this.calculationHistory.map(item => `
                    <div class="history-item">
                        <div class="history-header">
                            <span class="history-type">${item.data.type}</span>
                            <span class="history-time">${item.timestamp}</span>
                        </div>
                        <div class="history-details">
                            ${Object.entries(item.data).filter(([key]) => key !== 'type').map(([key, value]) => `
                                <span class="history-detail"><strong>${key}:</strong> ${value}</span>
                            `).join('')}
                        </div>
                    </div>
                `).join('')}
    ...
    saveCalculationHistory() {
        try {
            localStorage.setItem('healthcare_calculator_history', JSON.stringify(this.calculationHistory));
        } catch (e) {
    ...
    loadCalculationHistory() {
        try {
            const saved = localStorage.getItem('healthcare_calculator_history');
            if (saved) {
...
// Initialize the healthcare calculator
window.healthcareCalculator = new HealthcareMarketingCalculator();

// Auto-initialize when ContentLoader loads calculator content
function initializeCalculator() {
    if (document.querySelector('.calculator-container')) {
        if (window.healthcareCalculator && !window.healthcareCalculator.isInitialized) {
            window.healthcareCalculator.initializeCalculator();
        }
    }
}

// Initialize when the script loads if elements already exist
if (document.readyState === 'loading') {
